...
def _hour_label(minutes):
    hour = minutes // 60
    suffix = "a" if hour < 12 or hour == 24 else "p"
    return f"{(hour % 12) or 12}{suffix}"


def day_strip(user, day, include_planned=True, include_pending=False):
    """Positions for the horizontal day bar, as percentages of waking hours.

    Everything is worked out here so the template just drops the numbers into
    left/width styles.
    """
    start, end = waking_window(user)
    first, last = _to_minutes(start), _to_minutes(end)
    span = last - first

    def place(block_start, block_end):
        left = max(block_start, first)
        right = min(block_end, last)
        if right <= left:
            return None
        return round((left - first) / span * 100, 2), round((right - left) / span * 100, 2)

    segments = []
    for block in busy_blocks(user, day, include_planned=include_planned):
        placed = place(_to_minutes(block.start), _to_minutes(block.end))
        if placed:
            segments.append({"left": placed[0], "width": placed[1], "label": block.label,
                             "kind": block.source, "start": block.start, "end": block.end})

    if include_pending:
        for suggestion in user.suggestions.filter(date=day, status=Suggestion.Status.PENDING).select_related("backlog_item"):
            placed = place(_to_minutes(suggestion.start_time), _to_minutes(suggestion.end_time))
            if placed:
                segments.append({"left": placed[0], "width": placed[1], "label": suggestion.backlog_item.title,
                                 "kind": "pending", "start": suggestion.start_time, "end": suggestion.end_time})

    # A tick every three hours, starting on the first whole hour after waking.
    ticks = []
    tick = -(-first // 60) * 60
    while tick <= last:
        ticks.append({"left": round((tick - first) / span * 100, 2), "label": _hour_label(tick)})
        tick += 180

    now = None
    if day == timezone.localdate():
        current = _to_minutes(timezone.localtime().time())
        if first <= current <= last:
            now = round((current - first) / span * 100, 2)

    return {"segments": segments, "ticks": ticks, "now": now}
